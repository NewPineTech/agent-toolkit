...
import type { ChatProvider, ChatProviderConfig } from '../interfaces/chat-provider.interface.js';
...
class MockChatProvider implements ChatProvider {
  ...
  async *sendMessage(
    ...
    yield { type: 'done', sessionId: 'rf_mock_sess', providerSessionId: 'rf_mock_sess' };
  }
}
...
async function buildTestApp() {
  ...
  const chatProviderFactory = {
    create: () => ({
      ...
      config: { baseUrl: 'https://ragflow.mock', apiKey: 'sk-mock', agentId: 'agent_test' },
    }),
...
describe('Integration: Widget Routes', () => {
  ...
  describe('Rate limiting', () => {
    it('returns 429 when rate limit is exceeded', async () => {
      ...
      diContainer.register({
        config: asValue({ SESSION_TTL_MINUTES: 30, LOG_LEVEL: 'silent' } as any),
        encryptionService: asValue(encryption),
        tokenService: asValue(tokenService),
        domainValidator: asValue(new AllowlistDomainValidator()),
  ...
  describe('Error sanitization', () => {
    it('returns generic STREAM_ERROR when provider throws', async () => {
      ...
      const failingProvider: ChatProvider = {
        async createSession() { return 'rf_err_sess'; },
